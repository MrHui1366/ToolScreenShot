(function (exports, Laya) {
	...
	class SpineSkeleton extends Laya.Sprite {
	    ...
	    init(templet) {
	        ...
	        this.skeletonRenderer = window['conchUseSpine'] ? new ConchSpineSkeletonRenderer(false) : new SpineSkeletonRenderer(false);
	    ...
	    _update() {
	        ...
	        var delta = this.timeKeeper.delta * this._playbackRate;
	        var animationDuration = this.state.getCurrent(0).animation.duration;
	        this.currentPlayTime += delta;
	        while (animationDuration && this.currentPlayTime >= animationDuration)
	            this.currentPlayTime -= animationDuration;
	        if ((!!this._playStart && this.currentPlayTime < this._playStart) || (!!this._playEnd && this.currentPlayTime > this._playEnd)) {
	            this.state.update(this._playStart - this.currentPlayTime);
	            this.currentPlayTime = this._playStart;
	        }
	        else {
	    ...
	    addAnimation(trackIndex, nameOrIndex, loop = false, delay = 0) {
	        delay /= 1000;
	        var animationName = nameOrIndex;
	        if (typeof animationName == "number") {
	...
	class SpineTemplet extends Laya.Resource {
	    constructor(layaPremultipliedAlpha = true, spinePremultipliedAlpha = false) {
	        super();
	        this._textureDic = {};
	    ...
	    textureLoader(tex) {
	        ...
	        if (window['conchUseSpine']) {
	            tTexture._addReference();
	            tTexture.url = src;
	        }
	        return tTexture;
	    }
	    ...
	    destroy() {
	        ...
	        for (tTexture in this._textureDic) {
	            if (tTexture == "root")
	                continue;
	            if (tTexture) {
	                window['conchUseSpine'] && Laya.Laya.Loader.clearRes(this._textureDic[tTexture].url);
	...
	exports.ConchSpineSkeletonRenderer = ConchSpineSkeletonRenderer;
